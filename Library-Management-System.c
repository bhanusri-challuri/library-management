...
label1:
    ...
    char fname[255], lname[255], sid[15];
    char gender;
    double phone;
...
void modifyUser()
{
    ...
    char fname[255], lname[255], gender[5], sid[15];
    char fname1[255], lname1[255], gender1[5], sid1[15];
    double phone, phone1;
    ...
    while (fscanf(pT, "%s %s %s %s %lf \n", fname, lname, gender, sid, &phone) != EOF)
    {
        fprintf(pF, "%s %s %s %s %.0lf \n", fname, lname, gender, sid, phone);
    }
...
// third it checks if the book quantity is atleast 1, throws error if the book is OUT OF STOCK -- it also reduces book quantity by 1 from book_Records.txt
...
label4:
    ...
    char find[255];
    printf("Search the book by Author name or Book name: ");
    gets(find);

    FILE *pF = fopen("book_Records.txt", "r");

    while (fscanf(pF, "%s %s %s %lf %lf", name, author, publisher, &bookid, &quantity) != EOF)
    {

        if (strcmp(find, name) == 0 || strcmp(find, author) == 0)
        {
            printf("\n-------------------------\n");
            printf(">>> Record Found <<< \n");
            printf("-------------------------\n\n");
